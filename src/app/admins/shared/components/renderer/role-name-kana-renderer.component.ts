...
@Component({
    ...
    template: `
		<div *ngIf="this.params.data.typeName == 'user' && this.params.data.typeLabel" style="height: 100%; display: flex; align-items: center;">
    		...
    		<span *ngIf="this.params.data.typeName === 'user' && this.params.data.userDisable === 'on'" style="color: silver; padding-left:3px; padding-right:3px; word-break:break-all;">{{this.params.data.kana}}</span>
    		<span *ngIf="this.params.data.typeName === 'user' && this.params.data.userDisable === 'off'" style="color: black; padding-left:3px; padding-right:3px; word-break:break-all;">{{this.params.data.kana}}</span>
		</div>
    `,
    standalone: false
})
