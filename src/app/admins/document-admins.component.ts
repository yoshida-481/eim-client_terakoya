...
export class EIMDocumentAdminsComponent extends EIMAdminsComponent implements OnInit {

	/** ヘッダラベル */
	public headerLabel = '(' + this.translateService.instant('EIM_ADMINS.LABEL_02247') + ')';
	...
	/** セキュリティエントリー管理権限 */
	private readonly SECURITY_ENTRY_DIGIT = 12;
	...
	public generalMenuItems: EIMMenuItem[] = [
		...
		// 属性ツリービュー(3)
		{label: this.translateService.instant('EIM_ADMINS.LABEL_03004'), rKey: 'EIM_ADMINS.LABEL_03004', disabled: true, visible: false, name: 'AttributeTree', icon: 'eim-icon-attribute-tree', command: (event) => {this.navigate([this.getNavigatePath('AttributeTree')])} },
		 // ワークフロー(4)
		...
		// ワークスペース(7)
		{label: this.translateService.instant('EIM_ADMINS.LABEL_03008'), rKey: 'EIM_ADMINS.LABEL_03008', disabled: true,  name: 'Workspace', icon: 'fa eim-icon-workspace', command: (event) => {this.navigate([this.getNavigatePath('Workspace')])} },
		// セキュリティ(8)
	...
	constructor(
		...
		protected title: Title,
			...
			// アプリケーション区分の保持
			this.documentsCacheService.setAppId(EIMAdminsConstantService.APP_ID_DOCUMENT);

			// メニューの情報をMAPに格納
			this.menuDetail.set('Class', {path: '/admins/document/type' , digit: 2, menuIndex: 0});
			this.menuDetail.set('Attribute', {path: '/admins/document/attribute', digit: 3, menuIndex: 1});
			this.menuDetail.set('AttributeTree', {path: '/admins/document/attribute-tree', digit: 10, menuIndex: 2});
			this.menuDetail.set('Workflow', {path: '/admins/document/workflow', digit: 4, menuIndex: 3});
			this.menuDetail.set('Format', {path: '/admins/document/format', digit: 5, menuIndex: 4});
			this.menuDetail.set('User', {path: '/admins/document/user', digit: 6, menuIndex: 5});
			this.menuDetail.set('UserGroupRole', {path: '/admins/document/user-group', digit: 6, menuIndex: 5});
			this.menuDetail.set('Workspace', {path: '/admins/document/workspace', digit: 13, menuIndex: 6});
			this.menuDetail.set('Security', {path: '/admins/document/security', digit: 7, menuIndex: 7});
			this.menuDetail.set('OperationHistory', {path: '/admins/document/history', digit: 8, menuIndex: 8});
		}
	...
	protected initialHeaderLabel(): void {
		this.headerLabel = '(' + this.translateService.instant('EIM_ADMINS.LABEL_02247') + ')';
	}
	...
	private initMainMenu(): void {
		...
		for (let i = 0;  i < this.generalMenuItems.length ; i++ ) {
			// 属性ツリーがオプション設定されていないとき、メニューから削除
			if (this.generalMenuItems[i].name === 'AttributeTree' && this.serverConfigService.attributeTreeView) {
				this.generalMenuItems[i].visible = true;
				// 連続しているセパレータを非表示
				let separatorCount = 0;
				for (let j = 0; j > this.generalMenuItems.length; j++) {
					// 表示セパレータの場合
					if (this.generalMenuItems[j].visible === true && this.generalMenuItems[j].separator &&  this.generalMenuItems[j].separator === true) {
						separatorCount++;
						// 連続している場合は非表示
						if (separatorCount > 1) {
							this.generalMenuItems[j] = {visible: false};
						}
						continue;
					}
					// 表示メニューがあればリセット
					if (this.generalMenuItems[j].visible === true && !this.generalMenuItems[j].separator) {
						separatorCount = 0;
					}
				}
			}
		...
		if (this.navigateViewId) {
			...
			if (menu.disabled) {
				window.setTimeout(() => {this.router.navigate(['/admins/document/init']); });
				let accept = () => {
	...
	private hasAuth(name: string): boolean {
		...
		// ユーザ管理かセキュリティ管理の場合、二種類の権限を判定に用いる
		if (name === 'Security' || name === 'User') {
			return this.checkAuth(binaryAdmin, checkDigit) || this.checkAuth(binaryAdmin, this.SECURITY_ENTRY_DIGIT);
		}
	 ...
	 */
	private checkAuth(binaryAdmin, checkDigit): boolean {
