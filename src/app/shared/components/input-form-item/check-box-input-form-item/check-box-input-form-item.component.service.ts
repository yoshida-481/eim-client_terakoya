...
import { EIMCheckBoxInputFormItemComponent } from "./check-box-input-form-item.component";

export interface EIMCheckBoxInputFormItemComponentDomain extends EIMInputFormItemComponentDomain {
	options?: any;

};

/**
 * チェックボックス入力フォームコンポーネントサービス
 * @see {@link EIMInputFormItemComponentService}
 */
@Injectable()
export class EIMCheckBoxInputFormItemComponentService extends EIMInputFormItemComponentService {
	...
	public override getComponent(): Type<EIMInputFormItemComponent> {
		return EIMCheckBoxInputFormItemComponent;
	};
	...
	public override visibleFunction(inputFormItem: EIMInputFormItemDomain): boolean {
		...
		if (inputFormItem.uiControlId === 'uIControlCheckBox') {
			return true;
		}
		return false;
	};
	 ...
	 * @param inputFormItem 入力フォーム情報
	 */
	public override initializeComponent(inputFormItemComponent: EIMCheckBoxInputFormItemComponent, inputFormItem: EIMInputFormItemDomain): void {
	 ...
	 */
	public override createInputFormDomain(inputFormItemComponent: EIMCheckBoxInputFormItemComponentDomain): EIMInputFormItemDomain {
		let inputFormItem = super.createInputFormDomain(inputFormItemComponent);

		inputFormItem.options = inputFormItemComponent.options;

		inputFormItem.uiControlId = 'uIControlCheckBox'
