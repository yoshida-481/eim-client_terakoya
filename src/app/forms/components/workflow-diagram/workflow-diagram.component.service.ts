...
export class EIMWorkflowDiagramComponentService extends EIMDiagramComponentService {
	...
	/** ステータスタイプの高さ */
	static readonly STATUS_TYPE_HEIGHT: number = 35;
	/** ステータスタイプ間の距離 */
	...
	public show(info: EIMDiagramComponentInfo, param: any): void {
		...
		let workflow: EIMWorkflowDomain = param.workflow;
		if (!workflow) {
			this.clear(info);
			return;
		}
		let elements: EIMDiagramElement[] = this.createDiagramElement(info, workflow, {currentStatus: param.form.status});
		this.setData(info, elements, null);
		info.cy.resize();
		
		let viewWidth: number = info.component.element.clientWidth;
		let renderedPosition: any = info.cy.$("#" + param.form.status.type.id).renderedPosition();
		let panX: number = 100;
		let targetRightX: number = renderedPosition.x + EIMWorkflowDiagramComponentService.STATUS_TYPE_WIDTH * info.cy.zoom() + panX;
	...
	public createDiagramElement(info: EIMDiagramComponentInfo, workflow: EIMWorkflowDomain, params: any): EIMDiagramElement[] {
		...
		for (let i = 0; i < workflow.statusTypeList.length; i++) {
			...
			elements.push(this.createNode({id: statusType.id, label: this.getLabel(
					statusType.name, EIMWorkflowDiagramComponentService.MAX_LABEL_STRING_LENGTH, EIMWorkflowDiagramComponentService.MAX_LABEL_ROW_LENGTH), 
					domain: statusType, classes: classes, 
					position: { 
						x: nodeCnt * (EIMWorkflowDiagramComponentService.STATUS_TYPE_WIDTH + EIMWorkflowDiagramComponentService.STATUS_TYPE_SPACE_WIDTH), 
						y: 65 }, 
					selectable: true }));
			nodeCnt++;
