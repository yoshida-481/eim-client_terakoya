...
import { EIMDropdownInputRendererComponentService } from "./dropdown-input-renderer.component.service";
...
@Component({
  ...
  template: `
    ...
    <ng-container *ngIf="editable">
      ...
      <span
        *ngIf="options.length === 1"
        [style]="{ height: '100%', width: '100%', marginLeft: '3px' }"
      >
        {{ options[0].label }}
      </span>
    </ng-container>
  ...
  styles: [
    `
      ::ng-deep eim-dropdown-input-renderer .p-inputtext.p-dropdown-label {
        height: 17px;
      }
    `,
  ],
...
export class EIMDropdownInputRendererComponent
  implements AgRendererComponent, OnInit, OnDestroy
{
  ...
  constructor(
    ...
    protected rendererComponentService: EIMDropdownInputRendererComponentService
  ) {}
  ...
  agInit(params: any): void {
    ...
    let cellRendererParams: EIMDropdownInputRendererComponentParam =
    ...
    this.editedValueString =
      this.values[0] === null ? "" : this.getLabel(this.values[0]);

    // editable
    this.editable = this.isEditable(
      params.data,
      cellRendererParams ? cellRendererParams.editableFunction : null
    );
  }
  ...
  public onChange(event): void {
    if (this.params.onChangeFunction) {
      this.params.onChangeFunction(event.value, {
        field: this.params.colDef.field,
        dto: this.params.data,
      });
    }
