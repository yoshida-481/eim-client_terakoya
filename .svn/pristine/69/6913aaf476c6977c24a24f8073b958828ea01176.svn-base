...
import { EIMUrlInputFormItemComponent } from "./url-input-form-item.component";

/**
 * URL入力フォームアイテム情報
 */
export interface EIMUrlInputFormItemComponentDomain extends EIMInputFormItemComponentDomain {
	taskName: string;
};


/**
 * URL入力フォームコンポーネントサービス
 * @see {@link EIMInputFormItemComponentService}
 */
@Injectable()
export class EIMUrlInputFormItemComponentService extends EIMInputFormItemComponentService {
	...
	public override getComponent(): Type<EIMInputFormItemComponent> {
		return EIMUrlInputFormItemComponent;
	};
	...
	public override visibleFunction(inputFormItem: EIMInputFormItemDomain): boolean {
		...
		if (inputFormItem.uiControlId === 'uIControlUrl') {
			return true;
		}
		return false;
	};
	 ...
	 * @param inputFormItem 入力フォーム情報
	 */
	public override initializeComponent(inputFormItemComponent: EIMUrlInputFormItemComponent, inputFormItem: EIMInputFormItemDomain): void {
		
		if (inputFormItem['taskName'] !== null) {
			inputFormItemComponent.taskName = inputFormItem['taskName'];
		}
	 ...
	 */
	public override createInputFormDomain(inputFormItemComponent: EIMUrlInputFormItemComponentDomain): EIMInputFormItemDomain {
		let inputFormItem = super.createInputFormDomain(inputFormItemComponent);

		inputFormItem.uiControlId = 'uIControlUrl';
		inputFormItem['taskName'] = inputFormItemComponent.taskName;
