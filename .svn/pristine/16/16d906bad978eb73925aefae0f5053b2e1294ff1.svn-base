 ...
 */
@Injectable()
export class EIMPortalAuthenticationGuard extends EIMAuthenticationGuard<EIMLoginCheckResult> {
	...
	/**
	 * ログイン中かどうかチェックします.
	 * @return Observable<EIMLoginCheckResult>
	 */
	protected checkLogin(): Observable<EIMLoginCheckResult> {
		...
		return this.httpService.post('/rest/authenticate/checkLogin.mvc', params, false, false)
			.pipe(map(res => {
				...
				this.cacheService.setLoginUser(res.value.user);
	...
	/**
	 * メモリキャッシュにログイン情報情報を追加します.
	 * @param res ログインチェック結果情報
	 */
