 ...
 * @example
 *
 *      <eim-workflow-creator>
 *      </eim-workflow-creator>
 */
@Component({
    selector: 'eim-workflow-creator',
    templateUrl: './workflow-creator.component.html',
    styleUrls: ['./workflow-creator.component.css'],
    providers: [
        { provide: EIMComponent, useExisting: forwardRef(() => EIMWorkflowCreatorComponent) }
    ],
    standalone: false
})

export class EIMWorkflowCreatorComponent implements OnInit, EIMCreatable {
	/** ワークフローフォーム */
	 ...
	 */
	public create(): void {
		let nameSpace = this.getNamespaceLabel( this.inputNamespace );
		this.workflowService.create(this.nameList, nameSpace).subscribe((workflow: EIMWorkflowDomain) => {
	...
	public creatable(): boolean {
		return this.workflowCreatorForm.valid;
	}
